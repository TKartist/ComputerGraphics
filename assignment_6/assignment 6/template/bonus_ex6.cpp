...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material)
{
    ...
    for (int light_num = 0; light_num < lights.size(); light_num++)
    {
        ...
        Ray shadow_ray(point + normal * 0.001f, light_direction);
        bool isShadow = false;

        for (int i = 0; i < objects.size(); i++)
        {
            Hit shadowHit = objects[i]->intersect(shadow_ray);
            // if the shadow ray hit something and that hit object isn't itself and the light source, we can assume that it is a shadow.
            if (shadowHit.hit == true && shadowHit.distance > 0 && shadowHit.distance < glm::distance(point, lights[light_num]->position))
            {
                isShadow = true;
                break;
            }
...
glm::vec3 trace_ray(Ray ray, int depth)
{
    ...
    for (int k = 0; k < objects.size(); k++)
    {
        ...
        if (hit.hit == true && hit.distance > 0 && hit.distance < closest_hit.distance)
            closest_hit = hit;
    }
...
void sceneDefinition(int x)
{
    ...
    blue_specular.reflectionLevel = 1.0f;
